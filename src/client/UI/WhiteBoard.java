...
public class WhiteBoard extends JFrame implements ActionListener {
	...
	/*
     *
	 * public static JTextArea recviveArea = new JTextArea(20,40); //发送信息显示框
	 ...
	 * 
	 * //接收信息框竖直滚动条自动出现 recviveScroll.setVerticalScrollBarPolicy(
	 ...
	 * 
	 * //接收信息框竖直滚动条自动出现 sendScroll.setVerticalScrollBarPolicy(
	 * JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED); sendArea.setLineWrap(true);
	 * //超出显示长度自动换行 sendAreaPanel.setLayout(new BorderLayout());
	 * 
	 ...
	 * SendButtonAreaPanel; }
	 * 
	 * public static JPanel craetSendButton(){ //聊天区关闭和发送按钮面板 JPanel buttonPanel
	 * = new JPanel(); JButton close = new JButton("关闭(C)"); JButton send = new
	 * JButton("发送(S)"); buttonPanel.add(close); buttonPanel.add(send);
	 * 
	 * send.addActionListener( new ActionListener(){ //发送事件监听
	 * 
