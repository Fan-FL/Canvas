...
public class WhiteBoard extends JFrame implements ActionListener {
    ...
    public JMenuItem newfile, openfile, savefile, exit;
    public JMenuItem helpin, helpmain, colorchoice, strokeitem;
    public Icon nf, sf, of, ex;
	...
	/*
     *
	 * public static JTextArea recviveArea = new JTextArea(20,40); //鍙戦�佷俊鎭樉绀烘
	 * public static JTextArea sendArea = new JTextArea(5,40);
	 * 
	 * public static JPanel createRecvArea(){ JPanel recviveAreaPanel = new
	 * JPanel(); JScrollPane recviveScroll = new JScrollPane(recviveArea);
	 * 
	 * //鎺ユ敹淇℃伅妗嗙珫鐩存粴鍔ㄦ潯鑷姩鍑虹幇 recviveScroll.setVerticalScrollBarPolicy(
	 * JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED);
	 * recviveArea.setEditable(false); recviveArea.setLineWrap(true);
	 * //瓒呭嚭鏄剧ず闀垮害鑷姩鎹㈣
	 * 
	 * recviveAreaPanel.add(recviveScroll); return recviveAreaPanel; }
	 * 
	 * public static JPanel createSendArea(){ JPanel sendAreaPanel = new
	 * JPanel(); JScrollPane sendScroll = new JScrollPane(sendArea);
	 * 
	 * //鎺ユ敹淇℃伅妗嗙珫鐩存粴鍔ㄦ潯鑷姩鍑虹幇 sendScroll.setVerticalScrollBarPolicy(
	 * JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED); sendArea.setLineWrap(true);
	 * //瓒呭嚭鏄剧ず闀垮害鑷姩鎹㈣ sendAreaPanel.setLayout(new BorderLayout());
	 * 
	 * //sendAreaPanel.add( createAssLabel() ,BorderLayout.CENTER);
	 * sendAreaPanel.add(sendScroll,BorderLayout.SOUTH); return sendAreaPanel; }
	 * 
	 * public static JPanel createSendButtonArea(){ JPanel SendButtonAreaPanel =
	 * new JPanel(); SendButtonAreaPanel.setLayout( new BorderLayout() );
	 * SendButtonAreaPanel.add( craetSendButton(),BorderLayout.EAST ); return
	 * SendButtonAreaPanel; }
	 * 
	 * public static JPanel craetSendButton(){ //鑱婂ぉ鍖哄叧闂拰鍙戦�佹寜閽潰鏉� JPanel buttonPanel
	 * = new JPanel(); JButton close = new JButton("鍏抽棴(C)"); JButton send = new
	 * JButton("鍙戦��(S)"); buttonPanel.add(close); buttonPanel.add(send);
	 * 
	 * send.addActionListener( new ActionListener(){ //鍙戦�佷簨浠剁洃鍚�
	 * 
	 * @Override public void actionPerformed(ActionEvent e) { try { reader(); }
	 * catch (IOException e1) { e1.printStackTrace(); } sendArea.setText(null);
	 * }
	 * 
	 * });
	 * 
